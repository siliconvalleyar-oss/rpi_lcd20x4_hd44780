...
void LCD::init() {
    ...
    bcm2835_delay(1);
    sendNibble(0x03);
    sendNibble(0x02);   // Cambiar a 4 bits

    sendCommand(0x28);   // 4 bits, 2 líneas, 5x8
    sendCommand(0x08);   // Display off
    sendCommand(0x01);   // Clear
    bcm2835_delay(2);
    sendCommand(0x06);   // Incremento cursor
...
void LCD::writeString(const std::string& text) {
    for (char c : text) sendData(static_cast<uint8_t>(c));
}

void LCD::print(const std::string& text) { writeString(text); }
void LCD::clear() { sendCommand(0x01); bcm2835_delay(2); }

// ------------------------------------------------------------------
// Nuevo método: definir carácter personalizado en CGRAM
// location: 0..7
// charmap: array de 8 bytes (cada byte define una fila de 5 bits)
// ------------------------------------------------------------------
void LCD::createChar(uint8_t location, const uint8_t* charmap) {
    if (location > 7) return;
    // Comando para fijar la dirección de CGRAM: 0x40 + (location * 8)
    sendCommand(0x40 + (location * 8));
    for (int i = 0; i < 8; i++) {
        sendData(charmap[i]); // Los 5 bits menos significativos definen los píxeles
    }
    // Volver a la dirección de DDRAM (comando 0x80) - opcional
    sendCommand(0x80);
}
